...
const getPlacesByUserId = async (req, res, next) => {
  ...
  try {
    console.log(`Attempting to find places for user ID: ${userId}`);
    userWithPlaces = await User.findById(userId).populate('places');
    console.log('Found places:', userWithPlaces.places);
  } catch (err) {
...
const createPlace = async (req, res, next) => {
  ...
  try {
    console.log('Starting transaction to create place');
    ...
    console.log('Transaction successful, place created');
  } catch (err) {
...
const deletePlace = async (req, res, next) => {
  ...
  try {
    console.log('Starting transaction to delete place');
    const sess = await mongoose.startSession();
    sess.startTransaction();
    ...
    await sess.commitTransaction();
    console.log('Transaction successful, place deleted');
  } catch (err) {
    console.error('Error during transaction:', err);
    const error = new HttpError('Something went wrong, could not delete place.', 500);
    return next(error);
